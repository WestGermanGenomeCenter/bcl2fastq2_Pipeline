...
def getFastQCs(wildcards):
    ...
    if config["mapping"]["rseqc_active"]:
        fastQCs.extend(expand("{out}/rseqc/done.flag",out=outputfolder)) 
    if config["kraken2"]["kraken2_active"]:
...
rule blast:
    ...
    resources:
        threads=lambda wildcards, attempt: attempt * 18,
        time_hrs=lambda wildcards, attempt: attempt * 2,
...
rule jellyfish:
    ...
    shell:
        ...
        jellyfish count -m 21 -t {resources.threads} -s 100M  -o {params.mer_countsfile} -C {output.fastq} 2>{log}
